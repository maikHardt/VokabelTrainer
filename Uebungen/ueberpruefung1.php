...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($_POST as $key => $antwort) {
        if (strpos($key, 'vokabel') !== false) {
            $index = (int) filter_var($key, FILTER_SANITIZE_NUMBER_INT) - 1;
            $korrekteVokabeln[$index] = $antwort;

            // Hole die englische Übersetzung aus der Datenbank
            $vokabelwort = $antwort; // Aktuelle Vokabel
            $sql = "SELECT uebersetzung FROM vokabel WHERE vokabelwort = ? AND sprache_id = ? AND benutzer_id = ?";
            $stmt = $db->prepare($sql);
            $stmt->bind_param('sii', $vokabelwort, $sprache_id, $nutzerID);
            $stmt->execute();
            $result = $stmt->get_result();

            if ($row = $result->fetch_assoc()) {
                $uebersetzungen[$index] = $row['uebersetzung']; // Englische Übersetzung speichern
            }
        }
    ...
    foreach ($antworten as $index => $antwort) {
        $response['antworten'][] = [
            'vokabel' => $korrekteVokabeln[$index],
            'userAntwort' => $antwort,
